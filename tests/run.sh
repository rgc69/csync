...
test_proton_origin_deletion_detection() {
    ...
    assert_contains "$output_file" "R) Restore it by importing it into Calcurse" || return 1
    assert_contains "$output_file" "Press Enter to postpone and ask again next time" || return 1
    assert_not_contains "$output_file" "manual Proton deletion report" || return 1
    assert_contains "$output_file" "Decision postponed" || return 1
    assert_contains "$output_file" "The calendars are not fully synchronized" || return 1
    assert_contains "$output_file" "1 unresolved difference(s) remain" || return 1
    ...
    run_sync $'A\nR\ny\n'
    assert_status 0 || return 1
    assert_contains "$output_file" "It will be restored in Calcurse" || return 1
    event_has_line "$backup_dir/calendario.ics" "Proton New Event" "BEGIN:VALARM" || return 1
    assert_event_state_count 1 || return 1
}

test_calcurse_origin_deletion_detection() {
    ...
    # Simulate an export created before event-state.tsv was introduced. The
    # generated Proton file must provide a safe UID-based migration fallback.
    printf 'calcurse-sync-event-state\t1\n' > \
        "$test_home/.local/state/calcurse-sync/event-state.tsv"
    assert_event_state_count 0 || return 1

    # Simulate importing the generated file into Proton and deleting the event
    # from Calcurse before another synchronization observes both copies.
    : > "$calcurse_dir/apts"
    ...
    # As long as Proton still contains the event, every subsequent run must
    # continue to identify it as previously synchronized.
    cp "$proton_copy" "$backup_dir/My Calendar-test.ics"
    run_sync $'A\n\n'
    assert_status 0 || return 1
    assert_contains "$output_file" "Previously synchronized event no longer present in Calcurse" || return 1
    ...
    # Once the fresh Proton export no longer contains the event, the baseline
    # forgets it and no further decision is requested.
    install_proton_fixture empty.ics
    run_sync $'A\n'
    assert_status 0 || return 1
    assert_not_contains "$output_file" "Previously synchronized event no longer present in Calcurse" || return 1
    assert_contains "$output_file" "No changes to apply. The calendars are synchronized!" || return 1
    assert_event_state_count 0 || return 1
}
...
run_test "Proton-origin deletion detection" test_proton_origin_deletion_detection
