...
PROTON_DELETION_REPORT="$BACKUP_DIR/eventi-da-cancellare-proton.txt"
DRY_RUN=0
...
refresh_proton_deletion_report() {
    local keys_name="$1"
    local events_name="$2"
    local uids_name="$3"
    local dtstarts_name="$4"
    local -n keys="$keys_name"
    local -n events="$events_name"
    local -n uids="$uids_name"
    local -n dtstarts="$dtstarts_name"

    if [[ ${#keys[@]} -eq 0 ]]; then
        rm -f "$PROTON_DELETION_REPORT"
        return 0
    fi

    local tmp
    tmp=$(mktemp "$BACKUP_DIR/.proton-deletions.XXXXXX") || return 1
    chmod 600 "$tmp" 2>/dev/null || true

    {
        echo "Events to delete manually from Proton Calendar"
        echo "Generated by calcurse-sync: $TODAY"
        echo ""
        echo "This is not an ICS import file. Delete the following events in Proton,"
        echo "then export the updated Proton calendar and run Option A again."
        echo ""

        local index=0 pkey value summary start
        for pkey in "${keys[@]}"; do
            ((index++))
            value="${events[$pkey]}"
            summary="${value%%||*}"
            start="${dtstarts[$pkey]#*:}"
            echo "$index. ${summary:-[No title]}"
            echo "   Start: ${start:-unknown}"
            echo "   UID: ${uids[$pkey]:-unknown}"
            echo ""
        done
    } > "$tmp" || {
        rm -f "$tmp"
    ...
    if ! mv -f "$tmp" "$PROTON_DELETION_REPORT"; then
        rm -f "$tmp"
        return 1
    fi
}
...
option_A() {
   ...
   # echo "🔍 Analizzo le differenze tra i calendari..."
    ...
    declare -a events_to_delete_from_proton
    # ============================================================
    ...
    for key in "${!proton_events[@]}"; do
        ...
        if [[ $found_in_calcurse -eq 0 ]]; then
            ...
            if [[ $event_state_loaded -eq 1 && -n "${previous_event_state[$event_state_key]}" ]]; then
            ...
            if [[ $was_previously_synchronized -eq 1 ]]; then
                echo "   What do you want to do?"
                echo "   D) Add it to the manual Proton deletion report"
                echo "   R) Restore it by importing it into Calcurse"
                echo "   S) Postpone and ask again next time"
                echo ""
                read -rp "   Choice (D/R/S): " deletion_choice

                case "${deletion_choice^^}" in
                    D)
                        events_to_delete_from_proton+=("$key")
                        echo "   ✅ It will be added to the Proton deletion report"
                        ;;
                    R)
    ...
    if [[ ${#events_to_import_to_calcurse[@]} -eq 0 ]] && \
       ...
       [[ ${#events_to_delete_from_proton[@]} -eq 0 ]] && \
       ...
       [[ ${#alarm_removals_to_apply[@]} -eq 0 ]]; then
        ...
        if [[ $DRY_RUN -eq 1 ]]; then
            ...
            if ! save_event_state next_event_state; then
                warn "Calendars are synchronized, but the event state could not be updated."
            fi
            if ! refresh_proton_deletion_report events_to_delete_from_proton proton_events proton_uid_by_key proton_dtstart_line_by_key; then
                warn "The stale Proton deletion report could not be cleared."
            fi
        fi
    ...
    if [[ ${#events_to_delete_from_proton[@]} -gt 0 ]]; then
        echo "🗑️  Events to delete manually from Proton: ${#events_to_delete_from_proton[@]}"
        for key in "${events_to_delete_from_proton[@]}"; do
            local pval="${proton_events[$key]}"; local summary="${pval%%||*}"
            echo "   • ${summary:-[Senza titolo]}"
        done
        echo ""
    fi
    ...
    if ! refresh_proton_deletion_report events_to_delete_from_proton proton_events proton_uid_by_key proton_dtstart_line_by_key; then
        warn "The Proton deletion report could not be updated."
    elif [[ ${#events_to_delete_from_proton[@]} -gt 0 ]]; then
        echo ""
        echo "✅ Proton deletion report generated: $PROTON_DELETION_REPORT"
        echo "   📌 Delete these events manually in Proton, then export Proton again."
    fi
    ...
    echo "   • Events to delete manually from Proton: ${#events_to_delete_from_proton[@]}"
    echo ""
